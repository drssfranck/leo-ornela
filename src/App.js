  ...
  <div className='row'>
    ...
    <div className="col-lg-6">
      <div className="p-5 mt-4">
          ...
          <div>
          <ListGroup>
              <ListGroup.Item><b>16h00</b> : Accueil des invités par les mariés et leurs familles</ListGroup.Item>
              <ListGroup.Item><b>16h15</b> : Discours des mariés</ListGroup.Item>
              <ListGroup.Item><b>16h30</b> : Buffet et animations</ListGroup.Item>
              <ListGroup.Item><b>17h00 </b>: Coupe du gâteau</ListGroup.Item>
...
const Confirm = ({ id, open, handleConfirm }) => {
const x = document.getElementById(id);
const y = document.getElementById(id+'mbe');
if (x) {
  x.addEventListener("click",function(){
    y.style.display ='block';
    x.setAttribute('disabled','disabled')
    console.log('BOnjour mbe ! '+id)
    alert('Voulez vous valider ce cadeau ?' +data[id].name)
  });
    ...
    <div className=''>
      {
        data.map(mbe=>(
          ...
          <Figure className='maiva' key={mbe.id} >
            <Figure.Image className='image-cadeau'
              alt="171x180"
              src={mbe.img}
              
            />
            <p id={mbe.id+'mbe'} className='mbeselec' >Cadeau déjà selectioné</p>
            <Figure.Caption>
                <p className='titre-cadeau'>
                  {mbe.name}
                </p>
                <Button className='conf' id={mbe.id} onClick={() => setOpen(true)}>Choisir</Button>
                <Confirm 
                  id={mbe.id}
                  open={open}
